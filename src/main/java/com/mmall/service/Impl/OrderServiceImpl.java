...
public class OrderServiceImpl implements IOrderService {
    ...
    private long generateOrderNo(){
        ...
        return currentTime+new Random().nextInt(100);    //随机生成0到99随机数
    }
    ...
    private void dumpResponse(AlipayResponse response) {  // 简单打印应答
        if (response != null) {
    ...
    public ServerResponse aliCallback(Map<String,String> params){
        ...
        if(order == null){
            return ServerResponse.createByErrorMessage("非快乐mall的订单，回调忽略");
        }
    ...
    public ServerResponse<String> cancel(Integer userId,Long orderNo){
        ...
        if(order.getStatus() != Const.OrderStatusEnum.NO_PAY.getCode()){
            return ServerResponse.createByErrorMessage("已付款无法取消");
        }
    ...
    public void closeOrder(int hour) {
        ...
        for(Order order : orderList){
            ...
            for(OrderItem orderItem : orderItemList){
                //一定用主键where条件，防止锁表。且用InnoDB引擎
                Integer stock = productMapper.selectStockByProductId(orderItem.getProductId());

                //考虑到已经生成的订单里的产品，被删除的情况
                if(stock==null){
                    continue;
                }
